...
void Scene::loadModel(int idDOShader, std::string filePath) {
	m_DObj.at(idDOShader)->loadModel(filePath);
}
...
void Scene::makeRoateConst(int idDOShader, int moID) {
	m_DObj.at(idDOShader)->makeRoateConst(moID);
}
...
void Scene::setLightConeProperty(int idDOShader, int lightID, glm::vec3 lightDir, float innerCutoff, float outerCutoff ) {
	m_light[idDOShader]->setLightCone(lightID, lightDir, innerCutoff, outerCutoff);
}
...
void Scene::prepareTextureManager() {
	...
	m_TextureMan->loadTexture("house", "textures/house.png");
...
void Scene::prepareNightSky() {
	...
	addScaleObjModel(m_shaders.size() - 1, m_DObj.at(0)->getmodelCount() - 1, glm::vec3(600.0f));
...
void Scene::unloadAllTextures() {
	m_TextureMan->unloadAllTextures();
}
