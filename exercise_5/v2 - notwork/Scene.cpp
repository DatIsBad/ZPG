...
void Scene::addShaderOneColor() {
	m_shaders.push_back(new ShaderProgram(glm::vec3( (float)(std::rand() % 100) / (float)100, (float)(std::rand() % 100) / (float)100, (float)(std::rand() % 100) / (float)100) ));
}

void Scene::addShaderOneColor(float R, float G, float B) {
	m_shaders.push_back(new ShaderProgram( glm::vec3(R, G, B) ));
}

void Scene::addShader() {
	m_shaders.push_back(new ShaderProgram());
}

void Scene::addShader(const char* vertex_shader, const char* fragment_shader) {
	m_shaders.push_back(new ShaderProgram(vertex_shader, fragment_shader));
}
