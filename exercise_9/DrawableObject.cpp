...
void DrawableObject::loadModel(std::string filePath) {
	m_model->loadModel(filePath);
	m_tranl.push_back(new Transformation());
}


void DrawableObject::drawModels(ShaderProgram* shader, TextureManager* texture) {
	m_model->bind();

	
	for (int i = 0; i < m_model->getmodelCount(); i++) {
		shader->useModelMatrix(m_tranl.at(i)->getValue());
		m_model->drawModel( i , shader, texture);
	}
	m_model->unbind();
}

void DrawableObject::assignTexture(int id_model, std::string textureName, int textureMultiplier) {
	m_model->assignTexture(id_model, textureName, textureMultiplier);
}
...
void DrawableObject::makeRoateConst(int id_model) {
	m_tranl.at(id_model)->makeMeRotateConst(m_tranl.at(id_model)->getSize() - 1);
}
