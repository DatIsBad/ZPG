...
void main() {
	...
	for (int i = 0; i < numLights; i++) {
		...
		if (model.haveTexture == 1) {
			ambient = model.Ra * lights[i].ambientStrength * lightContribution * texture(model_texture, texPos).rgb;
		}
		else {
			ambient = model.Ra * lights[i].ambientStrength * lightContribution;
		}
		...
		vec3 diffuse;
		if (model.haveTexture == 1) {
			diffuse = model.Rd * diff * lightContribution * texture(model_texture, texPos).rgb;
		}
		else {
			diffuse = model.Rd * diff * lightContribution;
		}
		...
		vec3 specular = model.Rs * lights[i].specularStrength * spec * lightContribution;
