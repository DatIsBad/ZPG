...
void App::RunApp() {
	...
	scene1->addLight(0, 1, false, glm::vec3(3.0, 3.0, 10.0), glm::vec3(1.0, 1.0, 0.5), 0.9, 0.01f, 0.05f, true);
	scene1->setLightAttenuation(0, scene1->getSizeLight(0) - 1, -1, -1, -1, 5);
	scene1->addLight(0, 1, false, glm::vec3(3.0, 3.0, 3.0), glm::vec3(1.0, 1.0, 0.5), 0.9, 0.01f, 0.05f, true);
	scene1->setLightAttenuation(0, scene1->getSizeLight(0) - 1, -1, -1, -1, 5);
	scene1->addLight(0, 1, false, glm::vec3(2.0, 3.0, -2.0), glm::vec3(1.0, 1.0, 0.5), 0.9, 0.01f, 0.05f, true);
	scene1->setLightAttenuation(0, scene1->getSizeLight(0) - 1, -1, -1, -1, 5);
	scene1->addLight(0, 1, false, glm::vec3(0.0, 0.0, 0.0), glm::vec3(1.0, 1.0, 0.5), 0.9, 0.01f, 0.05f, true);
	scene1->setLightAttenuation(0, scene1->getSizeLight(0) - 1, -1, -1, -1, 5);

	// cone/reflector light attached to camera
	scene1->addLight(0, 2, true, glm::vec3(3.0, 3.0, 10.0), glm::vec3(1.0, 1.0, 1.0), 15.9, 0.01f, 0.05f, false);
	scene1->setLightAttenuation(0, scene1->getSizeLight(0) - 1, 2, 0.02, 0.02, 50);


	// cone/reflector light not attached to camera
	scene1->addLight(0, 2, false, glm::vec3(3.0, 0.3, -10.0), glm::vec3(1.0, 0.5, 0.5), 8.9, 0.1f, 0.15f, false);
	scene1->setLightAttenuation(0, scene1->getSizeLight(0) - 1, 2, 0.02, 0.03, 30);
	scene1->setLightConeProperty(0, scene1->getSizeLight(0) - 1, glm::vec3(1.0, -0.3, 0.5), 25.0, 45.0);


	scene1->addModel(0, "sphere");
	scene1->addScaleObjModel(0, scene1->getSizeDO(0) - 1, glm::vec3(0.2));
	scene1->addTranslationObjModel(0, scene1->getSizeDO(0) - 1, glm::vec3(3.0, 0.3, -10.0));


	// directional light
	scene1->addLight(0, 3, false, glm::vec3(0.0, 0.0, 0.0), glm::vec3(1.0, 1.0, 0.5), 0.8, 0.01f, 0.05f, false);


	glEnable(GL_TEXTURE_CUBE_MAP_SEAMLESS);
	scene1->prepareTextureManager();
	scene1->prepareNightSky();

	scene1->addModel(0, plain, std::size(plain));
	scene1->addScaleObjModel(0, (scene1->getSizeDO(0) - 1), glm::vec3(100.f));
	scene1->assignTexture(0, (scene1->getSizeDO(0) - 1), "grass", 100);
	...
	for (int i = 0; i < 50; i++) {
		...
		if (rand() % 6 == 0) {
			scene1->makeRoateConst(0, (scene1->getSizeDO(0) - 1));
		}
	...
	scene1->addModel(0, "suzi_flat");
	scene1->addTranslationObjModel(0, (scene1->getSizeDO(0) - 1), glm::vec3((float)(std::rand() % 2000 - 1000) / 100, 5.f, (float)(std::rand() % 2000 - 1000) / 100));
	
	scene1->addModel(0, "suzi_smooth");
	scene1->addTranslationObjModel(0, (scene1->getSizeDO(0) - 1), glm::vec3((float)(std::rand() % 2000 - 1000) / 100, 5.f, (float)(std::rand() % 2000 - 1000) / 100));
	
	
	scene1->loadModel(0, "models/datLog.obj");
	scene1->addScaleObjModel(0, scene1->getSizeDO(0) - 1, glm::vec3(20.0));
	scene1->addTranslationObjModel(0, scene1->getSizeDO(0) - 1, glm::vec3(0.0f, -50.0f, 0.0));
	scene1->addRotationObjModel(0, (scene1->getSizeDO(0) - 1), glm::radians((float)(90.0f)), glm::vec3(1.0f, 0.0f, 0.0f));


	scene1->loadModel(0, "models/house.obj");
	scene1->addTranslationObjModel(0, scene1->getSizeDO(0) - 1, glm::vec3(0.0f, 0.0f, 30.0));
	scene1->assignTexture(0, (scene1->getSizeDO(0) - 1), "house", 1);
	...
	scene2->addLight(0, 1, false, glm::vec3(0.0, 0.0, 0.0), false);
	...
	scene3->addLight(0, 1, false, glm::vec3(0.0, 0.0, 0.0), false);
	...
	scene4->addLight(0, 1, false, glm::vec3(0.0, 0.0, 0.0), false);
	...
	scene5->addLight(0, 1, false, glm::vec3(0.0, 0.0, 0.0), false);
	...
	while (!glfwWindowShouldClose(m_window)) {
		...
		for (int i = 0; i < m_scenes.size(); i++) {
			if (activeScene == i) {
				if (m_scenes.at(i)->m_active == false) {
					m_scenes.at(i)->m_active = true;
					if(i == 0)
						m_scenes.at(i)->prepareTextureManager();
				}
				m_scenes.at(i)->runScene();
			}
			else if(m_scenes.at(i)->m_active == true) {
				m_scenes.at(i)->m_active = false;
				m_scenes.at(i)->unloadAllTextures();
			}
		}
