...
ShaderProgram::ShaderProgram(int type, const Camera& cam):
	...
	if (type == 0) {
		...
		Shader::shaderCubeMap(m_shaderProgram);
	}
	...
	projLoc = glGetUniformLocation(m_shaderProgram, "projectionMatrix");
	cposLoc = glGetUniformLocation(m_shaderProgram, "cameraPos");
	cdirLoc = glGetUniformLocation(m_shaderProgram, "cameraDir");
	cmapLoc = glGetUniformLocation(m_shaderProgram, "uCubeMap");
}
...
void ShaderProgram::useLightShader(std::string baseName, glm::vec3 position, glm::vec3 color, float intensity, float ambientStrength, float specularStrength, 
		float constant, float linear, float quadratic, float range, 
		int type, glm::vec3 lightDir, float innerCutoff, float outerCutoff, bool enabled, bool isCamera) {
	...
	useInt((baseName + ".type").c_str(), type);
	useVector3fv((baseName + ".lightDir").c_str(), lightDir);
	useFloat((baseName + ".innerCutoff").c_str(), innerCutoff);
	useFloat((baseName + ".outerCutoff").c_str(), outerCutoff);
	useInt((baseName + ".enabled").c_str(), enabled);
	useInt((baseName + ".isCamera").c_str(), isCamera);
}


void ShaderProgram::useMaterial(glm::vec3 Ra, glm::vec3 Rd, glm::vec3 Rs ) {
	useVector3fv("model.Ra", Ra);	//ambient
	useVector3fv("model.Rd", Rd);	//diffuse
	useVector3fv("model.Rs", Rs);	//specular
}

void ShaderProgram::useHaveTexture(int haveTexture) {
	useInt("model.haveTexture", haveTexture);
}


void ShaderProgram::useCubeMap(GLuint id) {
	glUniform1i(cmapLoc, id);
}

void ShaderProgram::useTexture(GLuint id, int repeatTexture) {
	useInt("model_texture", id);
	useInt("repeatTexture", repeatTexture);
...
void ShaderProgram::useInt(const char* name, int value) {
